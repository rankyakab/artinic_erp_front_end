       ...
       // list.process &&
        ...
        return (
          <List key={key} disablePadding sx={{ px: 2 }}>
            ...
            {group.items.map(
              (list) => prev.includes(list.process) && (<><NavList key={list.title + list.path} data={list} depth={1} hasChild={!!list.children} />{console.log('hello process', list.process)} {console.log('hello process', list)}</>)
            )}
          </List>
        );
      })}
